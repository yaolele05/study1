...
void run( allline& pl)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        signal(SIGTTOU, SIG_DFL);  // 恢复终端输出信号
            ...
            {
                int fd = open(pl.cmds[i].outf.c_str(),
                    O_WRONLY | O_CREAT | (pl.cmds[i].append ? O_APPEND : O_TRUNC),
                    0644);
...
signal(SIGCHLD, (sighandler_t)reap);
    ...
    while (true)
    {
        ...
        if (!getline(cin, line)) {if (cin.eof()) break; continue;};
        ...
        if (pl.cmds.size() == 1)
        {
            ...
            if (cmd.empty() == false)
            {
                ...
                {
                    int fd = open(pl.cmds[0].outf.c_str(),
                        O_WRONLY | O_CREAT | (pl.cmds[0].append ? O_APPEND : O_TRUNC),
                        0644);
                    if (fd != -1)
                    {
                        dup2(fd, STDOUT_FILENO);
